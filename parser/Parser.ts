...
function checkSectionHeader(line: string): Section {
  ...
  if (line.match(/^(otras operaciones|otras op|otrasop)$/i)) {
...
export function parseOperacion(
  ...
  const args = _args.split(/×|✕/).map((arg) => arg.trim()).filter((arg) =>
    arg !== ""
  );
  let ithSlot = 0;
...
export function parseTad(source: string, context?: ParseContext): TAD | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (section === "none") {
      ...
      const generos = line.slice("generos".length).split(",").map((g) =>
        g.trim()
      ).filter((g) => g.length);
